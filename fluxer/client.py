        ...
        match event_name:
            case "READY":
                ...
                for guild_data in data.get("guilds", []):
                    ...
                    for ch_data in data.get("members", []):
                        member = GuildMember.from_data(ch_data, self._http, guild_id=guild.id)
            ...
            case "GUILD_CREATE":
                ...
                for member_data in data.get("members", []):
                    member = GuildMember.from_data(member_data, self._http, guild_id=guild.id)
                    self._users[member.user.id] = member.user
            ...
            case "GUILD_MEMBER_ADD":
                member = GuildMember.from_data(data, self._http, guild_id=int(data.get("guild_id")) if data.get("guild_id") else None)
            ...
            case "GUILD_MEMBER_REMOVE":
                ...
                if data.get("guild_id"):
                    # Update guild members cache (if the guild is cached)
                    guild = self._guilds.get(int(data["guild_id"]))
                    if guild:
